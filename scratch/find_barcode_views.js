...
async function main() {
    ...
    const views = await rpc(uid, 'ir.ui.view', 'search_read', [[
        ['name', 'ilike', 'barcode']
    ]], { fields: ['name', 'key', 'type', 'arch_base'] });
    
    console.log(`Found ${views.length} barcode views`);
    for (const v of views) {
        if (v.type === 'qweb') {
            console.log(`- QWEB: ${v.name} (${v.key})`);
            if (v.arch_base && v.arch_base.includes('Apply')) {
                console.log('  *** CONTAINS APPLY ***');
            }
        }
    }
}
