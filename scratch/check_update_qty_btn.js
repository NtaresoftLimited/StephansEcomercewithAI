...
async function main() {
    ...
    const views = await rpc(uid, "ir.ui.view", "search_read", [[["model", "=", "product.template"], ["type", "=", "form"]]], { fields: ["name", "arch_base"] });
    for (const v of views) {
        if (v.arch_base && v.arch_base.includes("action_update_quantity_on_hand")) {
            console.log(`Found in view: ${v.name}`);
            const lines = v.arch_base.split("\n");
            for (let i = 0; i < lines.length; i++) {
                if (lines[i].includes("action_update_quantity_on_hand")) {
