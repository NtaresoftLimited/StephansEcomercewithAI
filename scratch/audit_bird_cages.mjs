...
async function main() {
  ...
  const cageCats = [...birdCatIds].filter(id => { const c = allCats.find(x => x.id === id); return c && (c.name.toLowerCase().includes('cage') || c.name.toLowerCase().includes('home') || c.display_name.toLowerCase().includes('cage') || c.display_name.toLowerCase().includes('home')); });
  console.log("=== BIRD CAGE/HOME CATEGORIES IN ODOO ===");
