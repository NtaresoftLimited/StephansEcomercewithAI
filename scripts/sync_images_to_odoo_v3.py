...
def sync_images():
    ...
    print("Checking Odoo for existing images (small batches)...")
    batch_size = 20 # Smaller batch to avoid 502
    existing_images = {}
    for i in range(0, len(odoo_ids), batch_size):
        ...
        try:
            ...
            if i % 100 == 0:
                print(f"Checked {i} products...")
            time.sleep(0.5) # Slight delay
        except Exception as e:
            print(f"Error checking batch starting at {i}: {e}")
    ...
    for odoo_id in odoo_ids:
        ...
        try:
            # Download image
            img_resp = requests.get(p['imageUrl'], timeout=15)
            if img_resp.status_code == 200:
                ...
                time.sleep(1) # Delay between writes to avoid overloading
            else:
