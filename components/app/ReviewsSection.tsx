        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-y-12 gap-x-8 relative">
            ...
            {REVIEWS.map((review) => (
                <div key={review.id} className="flex flex-col text-center md:text-left px-4">
                    <div className="flex justify-center md:justify-start gap-1 mb-4 text-[#c77e35]">
                        {[...Array(5)].map((_, i) => (
                            <svg key={i} className="w-4 h-4 fill-current" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
                            </svg>
                        ))}
                    </div>
                    {/* Reduced text size here from text-xl md:text-2xl to text-base md:text-lg */}
                    <p className="text-base md:text-lg font-serif text-[#222] italic leading-relaxed mb-6">
                        "{review.text}"
                    </p>
                    <p className="text-xs text-[#666] mt-auto uppercase tracking-wider">— {review.name}</p>
                </div>
            ))}
        </div>
      </div>
