...
export type HeroImage = {
  ...
  _rev: string;
  title?: string;
  section?: "hero-home" | "hero-dog" | "hero-cat" | "hero-bird" | "hero-fish";
  image?: {
    asset?: {
      _ref: string;
      _type: "reference";
...
export type AllSanitySchemaTypes = Order | Product | SanityImageCrop | SanityImageHotspot | Slug | HeroImage | Customer | Category | SanityImagePaletteSwatch | SanityImagePalette | SanityImageDimensions | SanityImageMetadata | SanityFileAsset | SanityAssetSourceData | SanityImageAsset | Geopoint;
...
export type HERO_IMAGES_BY_SECTION_QUERYResult = Array<{
  _id: string;
  title: string | null;
  section: "hero-bird" | "hero-cat" | "hero-dog" | "hero-fish" | "hero-home" | null;
...
// Query: *[  _type == "product"  && (    name match $searchQuery + "*"    || description match $searchQuery + "*"  )] | score(  boost(name match $searchQuery + "*", 3),  boost(description match $searchQuery + "*", 1)) | order(_score desc) {  _id,  _score,  name,  "slug": slug.current,  "images": images[0...1]{    _key,    asset->{      _id,      url,      metadata    },    hotspot  },  category->{    _id,    title,    "slug": slug.current  },  material,  color,  stock}
...
// Query: *[  _type == "product"  && _id in $ids] {  _id,  name,  "slug": slug.current,  "images": images[0...1]{    _key,    asset->{      _id,      url,      metadata    }  },  stock}
