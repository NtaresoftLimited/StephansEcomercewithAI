...
const productSearchSchema = z.object({
  ...
  minPrice: z
    ...
    .optional()
    .default(0)
    .describe("Minimum price in TZS (e.g., 100000)"),
  maxPrice: z
    .number()
    .optional()
    .default(0)
    .describe("Maximum price in TZS (e.g., 500000). Use 0 for no maximum."),
});

export const searchProductsTool = tool({
  description:
    "Search for products in the pet store. Can search by name, description, or category, and filter by material, color, and price range. Returns product details including stock availability.",
  ...
  execute: async ({ query, category, material, color, minPrice, maxPrice }) => {
    ...
    try {
      ...
      const formattedProducts: SearchProduct[] = (
        products as any[] // AI_SEARCH_PRODUCTS_QUERYResult
      ).map((product) => ({
        id: product._id,
        ...
        category: product.categories?.[0]?.title ?? null,
        ...
        productUrl: product.slug ? `/shop/${product.slug}` : null,
      }));
